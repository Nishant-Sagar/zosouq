...
import { ShoppingCart, Search, Menu, X, Heart, LayoutGrid, Package, Loader } from 'lucide-react'
...
export default function Navbar() {
  ...
  useEffect(() => { setMobileOpen(false); setShowSug(false) }, [location])
  ...
  const fetchSuggestions = useCallback((q) => {
    ...
    debounceRef.current = setTimeout(() => {
      getProducts({ search: q.trim(), limit: 7 })
        .then(results => { setSuggestions(results || []); setShowSug(true) })
  ...
  const handleSearchFocus = () => {
    if (suggestions.length > 0 && searchQuery.trim().length >= 2) setShowSug(true)
  }
  ...
  const handleSearch = (e) => {
    e.preventDefault()
    if (searchQuery.trim()) goToSearch(searchQuery)
  }
  ...
  const handleKeyDown = (e) => {
    ...
    if (e.key === 'ArrowDown') { e.preventDefault(); setActiveIdx(i => Math.min(i + 1, suggestions.length - 1)) }
      ...
      e.preventDefault()
      navigate(`/product/${suggestions[activeIdx].slug}`)
      setShowSug(false); setSearchQuery(''); setSuggestions([])
    }
  }

  const SuggestionsDropdown = ({ isMobile = false }) => (
    showSug && (searchQuery.trim().length >= 2) ? (
      <div className={`absolute left-0 right-0 top-full mt-1 bg-white rounded-2xl shadow-2xl border border-gray-100 z-50 overflow-hidden ${isMobile ? '' : ''}`}>
        {/* Category shortcuts */}
        {categories.length > 0 && (
          <div className="px-3 pt-3 pb-1.5">
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider px-1 mb-1.5">Categories</p>
            <div className="flex flex-wrap gap-1.5">
              {categories.map(cat => (
                <Link key={cat.slug} to={`/category/${cat.slug}`}
                  onClick={() => { setShowSug(false); setSearchQuery('') }}
                  className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-gray-50 border border-gray-100 text-xs font-medium text-gray-600 hover:bg-pink-50 hover:text-pink-600 hover:border-pink-200 transition-all">
                  {cat.icon} {cat.name}
                </Link>
              ))}
            </div>
        ...
        {/* Divider */}
        {suggestions.length > 0 && <div className="mx-3 my-1.5 h-px bg-gray-100" />}
        ...
        {sugLoading ? (
          ...
          <>
            <div className="px-3 pt-0.5 pb-1">
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider px-1 mb-1">Products</p>
              {suggestions.map((p, i) => {
                ...
                return (
                  <Link key={p.id} to={`/product/${p.slug}`}
                    onClick={() => { setShowSug(false); setSearchQuery(''); setSuggestions([]) }}
                    className={`flex items-center gap-3 px-2 py-2 rounded-xl transition-all ${activeIdx === i ? 'bg-pink-50' : 'hover:bg-gray-50'}`}
                  >
                    <div className="w-10 h-10 rounded-lg bg-gray-100 flex-shrink-0 overflow-hidden">
                      {imgSrc
                        ? <img src={imgSrc} alt={p.name} className="w-full h-full object-contain p-0.5" loading="lazy" />
                        : <div className="w-full h-full bg-gray-200" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-medium text-gray-800 line-clamp-1">{p.name}</p>
                      <p className="text-[11px] text-gray-400 capitalize">{p.category?.name || ''}</p>
                    </div>
                    <div className="flex-shrink-0 text-right">
                      <p className="text-xs font-bold text-gray-900">KD {Number(p.price).toFixed(3)}</p>
                      {discount > 0 && <p className="text-[10px] font-semibold text-red-500">-{discount}%</p>}
                    </div>
                  </Link>
                )
              })}
            ...
            <div className="border-t border-gray-100">
              <button onClick={() => goToSearch(searchQuery)}
                className="w-full flex items-center justify-center gap-2 py-3 text-xs font-semibold text-pink-600 hover:bg-pink-50 transition-colors">
          ...
          <div className="px-4 py-5 text-center">
            <p className="text-xs text-gray-400">No products found for "<span className="font-medium text-gray-600">{searchQuery}</span>"</p>
          </div>
        )}
      </div>
    ) : null
      ...
      >
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6" dir="ltr">
            ...
            <form onSubmit={handleSearch} className="hidden sm:flex w-full">
              <div className="relative w-full" ref={searchRef}>
                ...
                <button type="submit" className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-pink-600 transition-colors">
                  {sugLoading ? <Loader className="w-4 h-4 animate-spin" /> : <Search className="w-4 h-4" />}
                </button>
                <SuggestionsDropdown />
              </div>
            </form>
          ...
          <div className="sm:hidden pb-2.5">
            <form onSubmit={handleSearch} className="relative" ref={!searchRef.current ? searchRef : undefined}>
